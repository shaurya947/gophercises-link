...
`
	expectedLinks := []Link{{Href: "/other-page", Text: "A link to another page"}}
	verifyExpectedLinks(t, html, expectedLinks)
}

func TestLinkWithNestedTags(t *testing.T) {
	html := `
<html>
<head>
...
func TestMultipleEmbeddedLinks(t *testing.T) {
	html := `
<!DOCTYPE html>
...
func TestLinkWithEmbeddedComment(t *testing.T) {
	html := `
<html>
<body>
...
`
	expectedLinks := []Link{{Href: "/dog-cat", Text: "dog cat"}}
	verifyExpectedLinks(t, html, expectedLinks)
}

func verifyExpectedLinks(t *testing.T, html string, expectedLinks []Link) {
	links, err := ParseFromReader(strings.NewReader(html))
	if err != nil {
		t.Errorf("Error parsing html: %+v", err)
	}

	if len(links) != len(expectedLinks) {
		t.Errorf("Expected %d links but got %d links", len(expectedLinks), len(links))
	}

	for idx := range links {
		if links[idx].Href != expectedLinks[idx].Href {
			t.Errorf("Expected link to be %s but got %s", expectedLinks[idx].Href, links[idx].Href)
		}
		if links[idx].Text != expectedLinks[idx].Text {
			t.Errorf("Expected link text to be %s but got %s", expectedLinks[idx].Text, links[idx].Text)
		}
	}
}
